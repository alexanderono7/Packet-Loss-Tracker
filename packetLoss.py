...
def ping(hours, inter, filePath):
    command = "timeout " + str(hours) + \
        "h fping 1.1.1.1 -l -b 4096 -p 33.3 -o -D -Q " + \
        str(inter) + " 2>&1 | tee " + filePath
    subprocess.call(
        command, shell=True)  # ping 1.1.1.1 with -b data size every 33.3 ms. // -o=timestamp // -D=delay // -Q=quiet // 2>&1=for pipe // tee=append
